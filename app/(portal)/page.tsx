...
async function getPublications(params: SearchParams): Promise<Publication[]> {
  ...
  if (params.de) query = query.gte('published_at', params.de)
  if (params.ate) query = query.lte('published_at', `${params.ate}T23:59:59`)
...
export default async function HomePage({
  ...
  return (
    <>
      {/* Hero */}
      <div className="bg-gradient-to-br from-brand-800 to-brand-950 text-white py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl sm:text-4xl font-bold mb-3">Transparência para todos</h2>
          <p className="text-brand-200 text-lg max-w-2xl mx-auto">
            Acesse atas, informações financeiras, comunicados, contratos e atualizações de obras.
          </p>
          <div className="mt-4 flex items-center justify-center gap-2 text-brand-300">
            <FileText className="w-4 h-4" />
            <span className="text-sm">
              {totalPublications} documento{totalPublications !== 1 ? 's' : ''} publicado{totalPublications !== 1 ? 's' : ''}
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="flex overflow-x-auto gap-0 divide-x divide-gray-200">
            {CATEGORIES.map((cat) => (
              <a
                key={cat}
                href={`/?categoria=${cat}`}
                className="flex-shrink-0 flex flex-col items-center py-3 px-5 hover:bg-gray-50 transition-colors group"
              >
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        {publications.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-lg font-semibold text-gray-600 mb-1">Nenhuma publicação encontrada</h3>
            <p className="text-gray-400 text-sm">Tente ajustar os filtros ou buscar por outro termo.</p>
            <a href="/" className="mt-4 inline-block text-brand-600 hover:text-brand-800 text-sm font-medium">
              Limpar filtros
            </a>
          </div>
        ) : (
